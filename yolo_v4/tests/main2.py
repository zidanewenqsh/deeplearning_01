...
class Trainer:
    def __init__(self, net=Net()):
        ...
        self.optimizer = optim.SGD(self.module.parameters(),lr=0.001)
    ...
    def train(self):
        ...
        for i in range(epoch):
            ...
            for j, (img_data, label) in enumerate(dataloader):
                ...
                # img_.save("./001.jpg")
                if i == 0 and j == 0:
                    a1 = self.module.state_dict()['inputs'].detach()
                    print("a1", a1[0])
                    print(self.module.state_dict())
                ...
                loss = self.loss_fn(y, label*35)
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()

                img_3 = transforms.ToPILImage(mode='L')(self.module.state_dict()["inputs"][1].detach())
        ...
        img_ = transforms.ToPILImage(mode='L')(self.module.state_dict()["inputs"][1])
        # print(torch.from_numpy(np.array(img_)))
        torch.save(self.module, 'net02.pth')
        img_.save("./000.jpg")
        a2 = self.module.state_dict()['inputs'].detach()
        print("a2", a2[0])
        print(torch.sum(img_data==a2),a1.shape)
